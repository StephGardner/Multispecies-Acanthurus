...
# Remove zOTUs from negative controls
contam <- read.csv("Contaminant.csv", header = FALSE)
ps <- prune_taxa(!(taxa_names(ps) %in% contam$V1), ps)
ps <- subset_samples(ps, Treatment != "Negative")

ps <- prune_taxa(taxa_sums(ps) > 1, ps) # prune zOTUs that are not present in any of the samples
...
p_jacc + geom_point(size = 6)  + theme_bw() + theme_classic() + 
  ...
  labs("Species") + scale_shape_manual(values = c(17, 19), name = "Shifting")
```
...
### Figure S7: PCoA with vector overlay
...
# Figure S7b
ggplot(plot_df, aes(Dim1, Dim2)) + geom_point(aes(shape = Shifting, color = Shifting, size = 6))  + theme_bw() + theme_classic() + 
  theme(text = element_text(size = 11)) + stat_ellipse(aes(color = Shifting, group = Shifting), level = 0.60) +
  labs("Shifting") + scale_shape_manual(values = c(17, 19), name = "Shifting") +
  geom_segment(data = vectors, aes(x = 0, xend = Dim1/5, y = 0, yend = Dim2/5), arrow = arrow(length = unit(0.2, "cm"))) +
  geom_text(data = vectors, aes(x = Dim1/5, y = Dim2/5, label = label), size = 4)
```
...
```{r}
glom <- tax_glom(ps.fish, taxrank = "phylum")
...
```{r}
glom <- tax_glom(ps.eam, taxrank = "phylum")
...
### Table S2: Relative abundances - fish, EAM, water
...
### Table S3: Relative abundances - fish species
...
A.dussumieri <- filter_taxa(A.dussumieri, function(x) mean(x) > 1, TRUE) # Filters out anything with a relative abundance under 1%
...
#View(relabun_fishspecies_order)
...
Shifting <- results(dsq, cooksCutoff = FALSE, contrast = c("Shifting", "Non_shifting", "Range_shifting"), tidy = TRUE) %>%
  ...
  mutate(test = "Non-range expanding v Range expanding")
...
gm_mean = function(x, na.rm = TRUE){  exp(sum(log(x[x > 0]), na.rm = na.rm) / length(x))} # calculate geometric means prior to estimate size factors

resultstax <- as.data.frame(tax_table(ps.fish)@.Data)
resultstax <- rownames_to_column(resultstax, var = "SampleID")

alpha <- 0.05
ps.core.shifting <- phyloseq_to_deseq2(ps.fish, ~ Shifting)
geoMeans <- apply(counts(ps.core.shifting), 1, gm_mean)
estszef <- estimateSizeFactors(ps.core.shifting, geoMeans = geoMeans)
dsq <- DESeq(estszef, fitType = "local")
resultsNames(dsq)
...
Shifting <- results(dsq, cooksCutoff = FALSE, contrast = c("Shifting", "Non_shifting", "Range_shifting"), tidy = TRUE) %>%
  ...
  left_join(., resultstax, by = c("row" = "SampleID")) %>%
  mutate(test = "Non-range expanding v Range expanding")
#write.csv(Shifting, file = "differential abundance_Shifting_ASV level.csv")
summary(Shifting)

error <- Shifting$lfcSE

ggplot(Shifting, aes(x = log2FoldChange, y = forcats::fct_rev(factor(order)))) +
...
### Table S5: - Fish core per SPECIES
100% core genera
...
```{r}
# FISH
glom <- tax_glom(ps.core, taxrank = "phylum")

trans <- transform_sample_counts(glom, function(x) x/sum(x)*100)
trans <- filter_taxa(trans, function(x) mean(x) > 0.01, TRUE)

long <- psmelt(trans) %>%
  group_by(OTU, phylum, Shifting, Species, Sample) %>%
  summarise(Relative_abun = sum(Abundance/n()),
            SE_rel = sd(Abundance)/sqrt(n()))

ggplot(long, aes(x = Sample, y = Relative_abun)) +
    geom_bar(stat = "identity", aes(fill = phylum), colour = "black", position = "stack") + 
    facet_wrap(~Shifting*Species, scales = "free_x", nrow = 1) +
...
# EAM
glom <- tax_glom(ps.core.eam, taxrank = "phylum")

trans <- transform_sample_counts(glom, function(x) x/sum(x)*100)
trans <- filter_taxa(trans, function(x) mean(x) > 0.01, TRUE)

long <- psmelt(trans) %>%
  group_by(OTU, phylum, Location, Site, Sample) %>%
  summarise(Relative_abun = sum(Abundance/n()),
            SE_rel = sd(Abundance)/sqrt(n()))

ggplot(long, aes(x = Sample, y = Relative_abun)) +
    geom_bar(stat = "identity", aes(fill = phylum), colour = "black", position = "stack") + 
    facet_wrap(~Location*Site, scales = "free_x", nrow = 1) +
    #scale_y_continuous(labels = scales::percent) +
    theme(aspect.ratio = 1, legend.position = "bottom", axis.text.x = element_blank()) +
...
# Figure S4 + Table S6: DESeq2 on core
...
alleam_core.melt <- psmelt(all_core.glom) %>%
 group_by(OTU, kingdom, phylum, class, order, family, genus) %>%
  summarise(Average_rel = mean(Abundance),
            SE_rel = sd(Abundance)/sqrt(n()))
#write.csv(x = alleam_core.melt, file = "core_100_site_genus_eam_venns.csv")

# Plot
ggplot(alleam_core.melt, aes(x = Site, y = Average_rel)) +
...
#write.csv(x = core_100_site_genus_eam, file = "core_100_site_genus_eam.csv")
...
## Figure S8: 
Core OTUs - percent shared EAM + fish species
```{r}
# https://www.datanovia.com/en/lessons/anova-in-r/
library(rstatix)
library(ggpubr)
...
Core OTUs - relative abundance of shared EAM + fish species
```{r}
# https://www.datanovia.com/en/lessons/anova-in-r/
library(rstatix)
library(ggpubr)
