...
Used to create Fig 1 EAM composition graphs, added onto map using Adobe Illustrator
```{r}
library(rstatix)
library(ggpubr)
...
p <- plot_ordination(trans, plotunifw, shape = "Shifting", color = "Species") +
...
p_jacc + geom_point(size = 6)  + theme_bw() + theme_classic() + 
  theme(text = element_text(size = 11)) + stat_ellipse(aes(group = Shifting), level = 0.80) +
  labs("Species") + scale_shape_manual(values = c(17, 19), name = "Expansion-mode")
```
...
#write.csv(Shifting, file = "differential abundance_Shifting.csv") ### Table S4a
...
## Run at zOTU level for Table S4b
...
#write.csv(Shifting, file = "differential abundance_Shifting_zOTU level.csv") ### Table S4b
```

# Fig. 5: Core microbiome
## FISH
100% core genera
```{r}
...
## CORE ADONIS
### Create ps.core 
using untransformed data for DESEq2

Run at the 100% OTU level fish species

```{r}
# FIRST - create the ps.core object (different from above, this is at zOTU level) - glom to genus at next step in DESeq2 code below chunk
...
# Fig. S1: Rareify data - ALL SAMPLES
...
## FISH
### Plot
USING RAREFIED (to the smallest, 10966)
...
## EAM
### Plot
USING RAREFIED (to the smallest, 4022)
...
# Species
beta.species <- betadisper(unifrac, sampledf$Species) 
boxplot(beta.species) # Figure 2d
...
WILL ONLY RUN IF PS.CORE IS UNTRANSFORMED - USING ABOVE CODE
...
Shift.core <- results(dsq, cooksCutoff = FALSE, contrast = c("Shifting", "Non_shifting", "Range_shifting"), tidy = TRUE) %>%
  ...
  left_join(., resultstax, by = c("row" = "SampleID")) %>%
  mutate(test = "Non_Shifting v Range_Shifting")
#write.csv(Shift.core, file = "differential abundance_Shifting_CORE.csv") ## Table S6

library("drlib")

# Plot - This is black and white plot, family along side
...
fish.content.summary <- fish.content %>%
  group_by(StomachContent, Species) %>% # Instead of species here, can also try GutSection, Shifting etc
  get_summary_stats(Content, type = "full") %>%
  mutate(mean_p_se = mean + se, mean_m_se = mean - se)

ggplot(fish.content.summary, aes(x = Species, y = mean)) +
    geom_bar(stat = "identity", aes(fill = StomachContent), position = "stack") +
    #geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.1, position = position_stack()) +
...
env_res <- envfit(pcoa_res, df_contents, permutations = 999) # These are the sig values to report in results
...
# Figure S8: STATS - Core OTUs - percent shared EAM + fish species
